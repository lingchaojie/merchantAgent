...
- 打包脚本先用 `electron-rebuild -f` 强制切到 Electron ABI。`win-unpacked` 可能与工作区 native binary 使用硬链接；因此发布矩阵必须先跑 Vitest、最后跑 `dist:dir`。之后若执行 `npm rebuild better-sqlite3` 恢复 Node ABI，需再跑一次 `dist:dir` 才能交付 unpacked 目录。
